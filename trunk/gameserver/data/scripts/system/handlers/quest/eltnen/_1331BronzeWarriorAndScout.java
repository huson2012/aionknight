...
public class _1331BronzeWarriorAndScout extends QuestHandler {

	private final static int questId = 1331;
	static QuestsData questsData = DataManager.QUEST_DATA;

	public _1331BronzeWarriorAndScout() {
		super(questId);
	}
	...
	public void register() {
		if (!CustomConfig.ENABLE_DAILYCOIN_EX) {
			qe.setNpcQuestData(203931).addOnQuestStart(questId);
			qe.setNpcQuestData(203931).addOnTalkEvent(questId);
		...
		if (targetId == 203931) {
			if (qs == null || qs.getStatus() == QuestStatus.NONE) {
				if (env.getDialogId() == 2) {
					...
					if (startPC == PlayerClass.SCOUT
							|| startPC == PlayerClass.WARRIOR) {
	...
	@Override
	public HandlerResult onBonusApplyEvent(QuestCookie env, int index,
			AbstractInventoryBonus bonus) {
		if (!(bonus instanceof CoinBonus))
			return HandlerResult.UNKNOWN;
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.REWARD) {
			if (index == 0 && qs.getQuestVarById(0) == 0 || index == 1
