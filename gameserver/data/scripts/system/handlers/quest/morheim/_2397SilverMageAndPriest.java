 ...
 * @author Orpheo
 */
public class _2397SilverMageAndPriest extends QuestHandler
{

	private final static int questId = 2397;
	static QuestsData questsData = DataManager.QUEST_DATA;

	public _2397SilverMageAndPriest()
	{
		super(questId);
	}
	...
	public void register()
	{
		...
		{
			qe.setNpcQuestData(204427).addOnQuestStart(questId);
			qe.setNpcQuestData(204427).addOnTalkEvent(questId);
		...
		if(targetId == 204427)
		{
			if(qs == null || qs.getStatus() == QuestStatus.NONE)
			{
				if(env.getDialogId() == 2)
				{
					...
					if(startPC == PlayerClass.PRIEST || startPC == PlayerClass.MAGE)
					{
			...
			else if(qs != null && qs.getStatus() == QuestStatus.START)
			{
				...
				{
					if(player.getInventory().getItemCountByItemId(186000008) >= 8)
					{
				...
				{
					if(player.getInventory().getItemCountByItemId(186000008) >= 16)
					{
				...
				{
					if(player.getInventory().getItemCountByItemId(186000008) >= 32)
					{
	...
	public HandlerResult onBonusApplyEvent(QuestCookie env, int index, AbstractInventoryBonus bonus)
	{
		...
		{
			if(index == 0 && qs.getQuestVarById(0) == 0 || index == 1 && qs.getQuestVarById(0) == 1 || index == 2 && qs.getQuestVarById(0) == 2 || index == 3 && qs.getQuestVarById(0) == 3)
